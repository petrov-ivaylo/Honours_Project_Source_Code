"""
Comparing the performances of D* search and Dijkstra's search in Vehicle Platooning problem with recharging points
...
import Platooning_with_D_star_search as dstar
import Platooning_with_Dijkstra_search as dijkstra
...
if __name__ == '__main__':
    ...
    for i in range(5,6,1):
        ...
        stations = [random.randint(0, 20) for _ in range(number_of_nodes)]  # Represents if there is a recharging station at a given node - if 0, then
                                                # there is no station, otherwise the number shows how much time (cost in minutes) it takes
        ...
        for j in range(20): # 20 tests are run for every type of input data in order to gather statistically well-informed results
            ...
            for vehicle in range(number_of_vehicles):
                ...
                tank_capacities.append(random.randint(80, 100)) # assigning a random integer between 80 and 100 for the max energy capacity for vehicle "vehicle"
            
            dstar_time, dstar_safe = dstar.main(start_end_points, tank_capacities, number_of_vehicles, stations, number_of_nodes, matrix_with_reduced_edges, covered_edges)
            
            matrix_with_reduced_edges = copy.deepcopy(original_matrix)
            covered_edges = set()
            
            dijkstra_time, dijkstra_safe = dijkstra.main(start_end_points, tank_capacities, number_of_vehicles, stations, number_of_nodes, matrix_with_reduced_edges, covered_edges)
            
            matrix_with_reduced_edges = copy.deepcopy(original_matrix)
            covered_edges = set()

            faster_algorithm = ""   # Stores which algorithm ran faster for the specific input data
            if dstar_time < dijkstra_time:
                faster_algorithm = "DStar"
            else:
                faster_algorithm = "Dijkstra"

            greater_safe = ""
            if dstar_safe > dijkstra_safe:
                greater_safe = "DStar"
            elif dijkstra_safe > dstar_safe:
                greater_safe = "Dijkstra"

            data.append([number_of_nodes, number_of_vehicles, dstar_time, dijkstra_time, dstar_safe, dijkstra_safe, faster_algorithm, greater_safe])
    ...
    df = pd.DataFrame(data, columns=['Number of nodes', 'Number of vehicles', 'D* search time', 'Dijkstra time search', 'D* search safe', 'Dijkstra search safe', 'Faster Algorithm', 'Greater safe'])
    full_path = os.path.realpath(__file__)
    path, filename = os.path.split(full_path)
    directory = path + "\\Tests_for_D_star_vs_Dijkstra.xlsx"
