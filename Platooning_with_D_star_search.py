"""
D* search for Vehicle Platooning problem with recharging points, where it is assumed that all but the leading vehicle make a 20% charge safe when being in a platoon
...
def main(start_end_points, tank_capacities, number_of_vehicles, stations, number_of_nodes, matrix_with_reduced_edges, covered_edges):
    ...
    for i in best_paths_without_platooning:
        ...
        dstar_object = DStar(number_of_nodes, matrix_with_reduced_edges, start_end_points[i][0], start_end_points[i][1])
        dstar_object.max_charge = tank_capacities[i]
        cost_with_platooning = dstar_find_best_path(dstar_object, corrected_stations, stations, matrix_with_reduced_edges, covered_edges, number_of_nodes, False)[0]
