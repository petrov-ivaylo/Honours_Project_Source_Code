"""
Comparing D* search implementing Ivaylo Petrov's idea and D* search implementing Elzbieta Futkowska's potential field method for heuristically solving the Vehicle Platooning problem with recharging points
...
import Ivaylo_D_star_with_recharging as ivaylo_dstar
import Elzbieta_Futkowska_Potential_Field_with_D_star_with_recharging as elzbieta_dstar
...
if __name__ == '__main__':
    ...
    for i in range(5,6,1):
        ...
        for j in range(20): # 20 tests are run for every type of input data in order to gather statistically well-informed results
            ...
            ivaylo_dstar_time, ivaylo_dstar_safe = ivaylo_dstar.main(start_end_points, tank_capacities, number_of_vehicles, copy.deepcopy(stations), copy.deepcopy(number_of_nodes), copy.deepcopy(matrix_with_reduced_edges), copy.deepcopy(covered_edges))
            
            matrix_with_reduced_edges = copy.deepcopy(original_matrix)
            covered_edges = dict()
            
            elzbieta_dstar_time, elzbieta_dstar_safe = elzbieta_dstar.main(copy.deepcopy(number_of_nodes), number_of_vehicles, start_end_points, tank_capacities, copy.deepcopy(stations), copy.deepcopy(original_matrix), copy.deepcopy(matrix_with_reduced_edges), copy.deepcopy(covered_edges))#(start_end_points, best_paths_without_platooning, number_of_vehicles, number_of_nodes, original_matrix, matrix_with_reduced_edges, covered_edges, [], [])
    ...
    directory = path + "\\Tests_for_Ivaylo_vs_Elzbieta_with_recharging.xlsx"
